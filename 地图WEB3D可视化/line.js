...
function createLoopLine(pointsArrs) {
  var group = new THREE.Group(); //一个国家多个轮廓线条line的父对象
  pointsArrs.forEach((polygon) => {
    var pointArr = []; //createLoopLine顶点坐标
    polygon[0].forEach((elem) => {
      pointArr.push(elem[0], elem[1], 0);
    });
    group.add(createLoopMesh(pointArr));
  });
  return group;
}

// pointArr：行政区一个多边形轮廓边界坐标(2个元素为一组，分别表示一个顶点x、y值)
function createLoopMesh(pointArr) {
  /**
